...
function draw() {
  let scalr = windowHeight / 1200;
  scale(scalr, scalr);
  let x1 = map(mouseX, scalr * 440, scalr * 750, 0, 200, true);
  let xl = map(mouseX, scalr * 90, scalr * 400, 50, 120, true);
  let xr = map(mouseX, scalr * 800, scalr * 1100, 0, 299, true);

  k.x = map(mouseX, windowHeight * 0.05, windowHeight * 1.1, -360, 1300, true);
