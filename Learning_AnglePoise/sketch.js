...
let blue = [255, 100, 100]; 
let silver = [220, 220, 230];
let black = [40, 40, 40];
let bulb = [255, 255, 120];
let bulboff = [180, 180, 180];
let lit = [255, 255, 170];
let unlit = [150, 150, 150];

function preload() {
  fontRegular = loadFont('BigShouldersDisplay-VariableFont_wght.ttf');
}
...
function draw() {
  ...
  // textStyle(BOLD);
  if (mouseX > width / 2 - 1 && mouseX < width / 2 + 1) {
    stroke(255, 70);
    strokeWeight(sca*3);
      fill(255);
    // filter(BLUR, 3);
  ...
  textSize(sca*700);
  text("A", sca*72, sca*348);
  text("N", sca*208,sca* 348);
  text("G", sca*372,sca* 348);
  text("L", sca*506, sca*348);
  text("E", sca*633, sca*348);

  textSize(sca*700);
  text("P", sca*-30, sca*923);
  text("O",  sca*110, sca*923);
  text("I",  sca*247, sca*923);
  text("S", sca*290, sca*923);
  text("E", sca*414, sca*923);
  
  stroke(0);
  strokeWeight(sca*1.1);
  fill(30);
  // rect(-10, height * (2.8 / 4), width + 20, height / 2);
  noCursor();
  strokeWeight(sca*1.4);

  let mapx = map(mouseX, 0, height, -1, 1);
  let randn = random(0, 0);
  var x1 = (width / 2 - mouseX) * (sca*18 * sin(mapx));
  var y1 = (height / 2 - mouseY) * sca*20 * sin(mapx);
  ...
  fill(255);
  textSize(11);
  text(width / 2 - mouseX, mouseX + 5, mouseY + 15);
  text(height / 2 - mouseY, mouseX + 5, mouseY + 25);
